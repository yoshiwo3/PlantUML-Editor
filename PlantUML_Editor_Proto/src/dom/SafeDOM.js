...
class SafeDOM {
    constructor(options = {}) {
        ...
        console.log('[SafeDOM] Initialized with options:', this.options);
    }
    ...
    checkDOMPurify() {
        if (this.options.enableDOMPurify && typeof window !== 'undefined' && window.DOMPurify) {
            ...
            console.log('[SafeDOM] DOMPurify detected and configured');
        } else {
            this.hasDOMPurify = false;
            console.warn('[SafeDOM] DOMPurify not found, using fallback sanitization');
        }
    }
    ...
    wrapEventHandler(handler, eventType) {
        return (event) => {
            try {
                ...
                console.error(`[SafeDOM] Event handler error:`, error);
            }
        };
    }
    ...
    logSecurityEvent(eventType, message, metadata = {}) {
        if (this.securityLogger) {
            ...
            console.warn(`[SafeDOM:${eventType}] ${message}`, metadata);
        }
    }
    ...
    destroy() {
        ...
        console.log('[SafeDOM] Destroyed');
    }
}

// DOMPurifyが利用可能でない場合のフォールバック警告
if (typeof window !== 'undefined' && !window.DOMPurify) {
    console.warn('[SafeDOM] DOMPurify is not available. Consider including it for enhanced security.');
    console.warn('[SafeDOM] Fallback sanitization is active but may be less secure.');
}
